...
# `gtsummarytools` <img src='man/figures/sticker.png' align="right" width="200">

`gtsummarytools` is a package to modify gtsummary tables notably by
including the number of non missing observations.
...
gtsummary tables of class “tbl_summary” show total Ns by default, which
can lead to confusion, especially with regards the calculation of
percentages in the presence of missing data.
...
the function `add_nobs` in `gtsummarytools` allow to add the Nobs
(i.e. number of non-missing data) to a table of class c(‘tbl_summary’,
‘gtsummary’), generated by `tbl_summary`. Depending on the specified
location Nobs will be:
